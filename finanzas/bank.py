...
def get_file_format():
    ...
    #deploy.extend([[x[0],x[1],x[2] + ' ' + " ".join(x[4].split()[:-2]), " ".join(x[4].split()[-2:]), '0', x[3], '0', '0'] for x in final if len(x) == 5 and 'TEF' in x[2] and 'TARJETA' in x[4] ])
    #deploy.extend([[x[0],x[1],x[2] + ' ' + " ".join(x[4].split()[:-2]), " ".join(x[4].split()[-2:]), '0', x[3], '0', '0'] for x in final if len(x) == 5 and 'TEF' in x[2] and 'TC' in x[4] ])
    deploy.extend([[x[0],x[1],x[2] + ' ' + " ".join(x[6].split()[:-1]), " ".join(x[6].split()[-1:]), '0', x[3], x[4], x[5]] for x in final if len(x) == 7 and 'HOTEL' in x[2] ])
    #deploy.extend([[x[0],x[1],x[2] + ' ' + " ".join(x[6].split()[:-2]), " ".join(x[6].split()[-2:]), '0', x[3], '0', '0'] for x in final if len(x) == 7 and 'TEF' in x[2] and 'TC' in x[4] ])



    deploy.extend([[x[0],x[1],x[2] + ' ' + " ".join(x[4].split()[:-2]), " ".join(x[4].split()[-2:]), '0', x[3], '0', '0'] for x in final if len(x) == 5 and 'SPEI' in x[2] and 'ORACLE' in x[4] ])
    ...
    deploy.extend([[x[0],x[1],x[2] + ' ' + " ".join(x[6].split()[:-1]) , x[6].split()[-1].lstrip(), x[3], '0', x[4], x[5] ] for x in final if len(x) == 7 and 'CFE' in x[2]])
